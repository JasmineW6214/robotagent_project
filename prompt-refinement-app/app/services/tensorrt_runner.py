"""
TensorRT-LLM Runner Service.

Runs VLM inference using TensorRT-LLM server for faster inference.
Requires TensorRT-LLM Docker server to be running separately.
"""
...
# Image resolution presets (same as vllm_runner.py)
...
@dataclass
class TensorRTRunResult:
    """Result from running TensorRT-LLM inference."""
    model: str
    raw_response: str
    parsed_output: dict
    success: bool
...
class TensorRTRunner:
    """Run VLM inference using TensorRT-LLM server with OpenAI-compatible API."""
    ...
    def __init__(
        ...
        Args:
            base_url: TensorRT-LLM server URL (default: http://localhost:8001)
    ...
    def is_available(self) -> bool:
        """Check if TensorRT-LLM server is running and accessible."""
        try:
    ...
    def get_models(self) -> list[str]:
        """Get list of available models from TensorRT-LLM server."""
        try:
            ...
            logger.warning(f"Failed to get models from TensorRT-LLM: {e}")
        return []
    ...
    def warm_cache(self, system_prompts: list[str]) -> bool:
        ...
        if not self.is_available():
            logger.warning("TensorRT-LLM server not available, skipping cache warm-up")
            return False
    ...
    def run(
        ...
        """Run VLM inference via TensorRT-LLM server.
        ...
        Returns:
            TensorRTRunResult with response and parsed output
        """
        ...
        if available_models:
            ...
            if actual_model != model:
                logger.info(f"Using TensorRT-LLM model: {actual_model} (requested: {model})")
            model = actual_model
        else:
            logger.warning(f"No models available from TensorRT-LLM server, using requested: {model}")
        ...
        try:
            ...
            if response.status_code != 200:
                error_msg = f"TensorRT-LLM server error: {response.status_code} - {response.text}"
                logger.error(error_msg)
                return TensorRTRunResult(
                    model=model,
                    raw_response="",
                    parsed_output={},
                    success=False,
            ...
            logger.info(f"TensorRT-LLM inference: {latency_ms:.0f}ms, "
            ...
            error_msg = f"Cannot connect to TensorRT-LLM server at {self._base_url}. Is it running?"
            logger.error(error_msg)
            return TensorRTRunResult(
                model=model,
                raw_response="",
            ...
            logger.error(f"TensorRT-LLM run error: {e}")
            import traceback
            traceback.print_exc()
            return TensorRTRunResult(
                model=model,
    ...
    def _prepare_image(self, image: Any, max_pixels: Optional[int] = None) -> Optional[str]:
        """Convert image to base64 data URL for TensorRT-LLM.
    ...
    def unload(self) -> dict:
        """No-op for TensorRT-LLM (model managed by server)."""
        return {"message": "TensorRT-LLM models are managed by the Docker server"}

    def list_available_models(self) -> list[str]:
        """List models available on TensorRT-LLM server."""
